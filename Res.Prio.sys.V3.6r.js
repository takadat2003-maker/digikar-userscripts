...
(function () {
  ...
  const CONFIG = {
    ...
    storagePrefixBase: 'tmPriorityStateV36r::',
  ...
  function getStoragePrefix() {
    return `${CONFIG.storagePrefixBase}${getTodayKey()}::`;
  }
  ...
  function getStorageKey(patientNo) {
    return `${getStoragePrefix()}${patientNo}`;
  }
  ...
  function isReturnGroupItem(item) {
    ...
    const memo = String(item.receptionMemoText || '');
    const dep = normalizeCompareText(item.departmentText || '');
    const status = normalizeCompareText(item.statusText || '');
    const inMiddle = dep === normalizeCompareText('中待合') || dep === normalizeCompareText('中待合室');
    return memo.includes('🔙再帰あり') && inMiddle && status === normalizeCompareText('診察待');
  }
  ...
  function updateTransitionState(patientNo, statusText, arrivalAt, nowMs, isReturnGroupNow) {
    if (!patientNo) return null;
    ...
    let returnWaitingStartAt = prev.returnWaitingStartAt || null;

    if (!totalStartAt) totalStartAt = arrivalAt ? arrivalAt.getTime() : nowMs;

    if (waitingNow) {
      if (!currentWaitingStartAt) currentWaitingStartAt = arrivalAt ? arrivalAt.getTime() : totalStartAt;
    } else {
      currentWaitingStartAt = null;
    }

    if (isReturnGroupNow) {
      if (!returnWaitingStartAt) returnWaitingStartAt = nowMs;
    } else {
      returnWaitingStartAt = null;
    }
  ...
  function cleanupMissingRows(seenPatientNos) {
    ...
    for (let i = 0; i < localStorage.length; i += 1) {
      ...
      if (seenPatientNos.has(patientNo)) continue;

      try {
        const raw = localStorage.getItem(k);
        const state = raw ? JSON.parse(raw) : null;
        const lastSeenAt = state && state.lastSeenAt ? Number(state.lastSeenAt) : 0;

        if (!lastSeenAt || now - lastSeenAt > CONFIG.staleStateTtlMs) {
          removeKeys.push(k);
        }
      } catch (e) {
        removeKeys.push(k);
  ...
  function cleanupOldDatePrefixes() {
    const todayPrefix = getStoragePrefix();
    const removeKeys = [];

    for (let i = 0; i < localStorage.length; i += 1) {
      const k = localStorage.key(i);
      if (!k) continue;
      if (!k.startsWith(CONFIG.storagePrefixBase)) continue;
      if (k.startsWith(todayPrefix)) continue;
      removeKeys.push(k);
    }
  ...
  function buildRowData(row, cols, now) {
    ...
    const depNorm = normalizeCompareText(departmentText);
    const isMiddleDept = depNorm === normalizeCompareText('中待合') || depNorm === normalizeCompareText('中待合室');
    const hasReturnTag = String(receptionMemoText || '').includes('🔙再帰あり');
    const isReturnGroupNow = waitingNow && isMiddleDept && hasReturnTag;

    if (trackableStatus) {
      updateTransitionState(patientNoText, statusText, arrivalAt, nowMs, isReturnGroupNow);
    }
    ...
    const state = loadState(patientNoText) || updateTransitionState(patientNoText, statusText, arrivalAt, nowMs, isReturnGroupNow) || {};
    const totalStartAtMs = state.totalStartAt || (arrivalAt ? arrivalAt.getTime() : nowMs);
    const currentWaitingStartAtMs = state.currentWaitingStartAt || (arrivalAt ? arrivalAt.getTime() : nowMs);

    const totalWaitMs = Math.max(0, nowMs - totalStartAtMs);
    const currentWaitMs = waitingNow ? Math.max(0, nowMs - currentWaitingStartAtMs) : 0;
    const returnWaitingStartAtMs = state.returnWaitingStartAt || null;
    const returnWaitMs = isReturnGroupNow && returnWaitingStartAtMs ? Math.max(0, nowMs - returnWaitingStartAtMs) : 0;
    ...
    detailParts.push(`待機加点+${formatScore(waitScore)}`);
    ...
    return {
      ...
      totalWaitMs,
      returnWaitMs,
      isReturnGroup: isReturnGroupNow,
      score: totalScore,
  ...
  function renderWaitingRow(item, rank) {
    ...
    const returnWaitStr = item.isReturnGroup ? formatDuration(item.returnWaitMs || 0) : '';
    ...
    const html =
      ...
      `</div>` +
      `<div style="margin-top:2px;font-weight:700;color:#7c3aed;">⌚総待ち${escapeHtml(totalWaitStr)}` +
      (item.isReturnGroup ? `<span style="margin-left:8px;color:#0f766e;">🔙再帰待${escapeHtml(returnWaitStr)}</span>` : '') +
    ...
    const signature = [
      ...
      item.isReturnGroup ? formatDuration(item.returnWaitMs || 0) : '',
      reservedStr,
  ...
  function renderPanel(waitingItems, examiningItems) {
    ...
    const frontWaitingCount = countFrontWaiting(waitingItems);
    const middleWaitingCount = countMiddleWaiting(waitingItems);
    const returnGroupCount = countReturnGroup(waitingItems);

    summaryEl.innerHTML =
      `<div style="line-height:1.5;">` +
      `<div><span style="color:${CONFIG.colors.examiningStatus};font-weight:800;">診察中 ${examCount}人</span></div>` +
      `<div>診察待 ${waitingItems.length}人（合計の待ち人数）</div>` +
      `<div>受付前待 ${frontWaitingCount}人</div>` +
      `<div>中待合待 ${middleWaitingCount}人</div>` +
      `<div>再帰群 ${returnGroupCount}人</div>` +
      `<div style="font-weight:700;color:${uiState.sortEnabled ? CONFIG.colors.sortOn : CONFIG.colors.sortOff};">${uiState.sortEnabled ? '本表ソート中' : '本表通常順'}</div>` +
      `</div>`;
    ...
    html += topItems.map((item, idx) => {
      ...
      return `
        <div title="${escapeHtml(item.detailText)}"
          ...
          <div style="margin-top:4px;font-size:12px;color:#111827;">${escapeHtml(item.departmentText || '')}</div>
          <div style="margin-top:4px;font-size:12px;color:#7c3aed;font-weight:700;">⌚総待ち${escapeHtml(totalWaitStr)}
            ${item.isReturnGroup ? `<span style="margin-left:8px;color:#0f766e;">🔙再帰待${escapeHtml(returnWaitStr)}</span>` : ''}
          </div>
  ...
  function periodicHouseKeeping() {
    const now = Date.now();
    if (now - lastGlobalGcAt >= CONFIG.globalGcIntervalMs) {
      lastGlobalGcAt = now;
