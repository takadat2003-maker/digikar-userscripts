...
// @version      3.6.1-r
// @description  診察順ナビ 上位表示パネル版。V3.6系の安定ソートを維持しつつ、再帰群判定を受付メモタグ基準に簡素化。
...
(function () {
  ...
  function hasReturnTag(receptionMemoText) {
    return String(receptionMemoText || '').includes(CONFIG.returnTag);
  }
  ...
  function isReturnGroupItem(item) {
    ...
    return hasReturnTag(item.receptionMemoText) &&
      isMiddleWaitingDepartment(item.departmentText) &&
      isWaitingStatus(item.statusText);
  }
  ...
  function updateTransitionState(patientNo, statusText, arrivalAt, nowMs, isReturnGroupNow) {
    ...
    let returnGroupStartAt = prev.returnGroupStartAt || null;
    ...
    if (isReturnGroupNow) {
      if (!returnGroupStartAt) {
        returnGroupStartAt = arrivalAt ? arrivalAt.getTime() : nowMs;
      }
    } else {
      returnGroupStartAt = null;
    }
  ...
  function buildRowData(row, cols, now) {
    ...
    const returnGroupNow = waitingNow && hasReturnTag(receptionMemoText) && isMiddleWaitingDepartment(departmentText);
    ...
    if (trackableStatus) {
      updateTransitionState(patientNoText, statusText, arrivalAt, nowMs, returnGroupNow);
    }
    ...
    const state = loadState(patientNoText) || updateTransitionState(patientNoText, statusText, arrivalAt, nowMs, returnGroupNow) || {};
    const totalStartAtMs = state.totalStartAt || (arrivalAt ? arrivalAt.getTime() : nowMs);
    const returnGroupStartAtMs = state.returnGroupStartAt || null;

    const totalWaitMs = Math.max(0, nowMs - totalStartAtMs);
    const currentWaitMs = waitingNow ? totalWaitMs : 0;
    const returnWaitMs = (returnGroupNow && returnGroupStartAtMs) ? Math.max(0, nowMs - returnGroupStartAtMs) : 0;
  ...
  function renderWaitingRow(item, rank) {
    ...
    const returnWaitStr = formatDuration(item.returnWaitMs || 0);
  ...
  function renderPanel(waitingItems, examiningItems) {
    ...
    summaryEl.innerHTML =
      ...
      `<span style="color:${CONFIG.colors.examiningStatus};font-weight:800;">診察中 ${examCount}人</span>` +
      `　<span style="font-weight:700;">診察待 ${waitingItems.length}人</span>` +
      `　<span style="font-weight:700;">受付前待 ${frontWaitingCount}人</span><br>` +
      `<span style="font-weight:700;">中待合待 ${middleWaitingCount}人</span>` +
      `　<span style="font-weight:700;color:#0f766e;">再帰群 ${returnGroupCount}人</span>` +
      ...
      `　<span style="color:#6b7280;">上位${topItems.length}名表示</span>` +
      `</div>`;
