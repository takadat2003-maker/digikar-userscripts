// ==UserScript==
// @name         Res.Prio.sys.V3.8
// @namespace    https://digikar.jp/reception/
// @version      3.8.0
// @description  診察順ナビ 上位表示パネル版。Bridge不要。件数サマリー（診察中/診察待/受付前待/中待合待/再帰群）を常時表示。
...
(function () {
  ...
  const CONFIG = {
    ...
    storagePrefixBase: 'tmPriorityStateV38::',
    staleStateTtlMs: 6 * 60 * 60 * 1000,
    globalGcIntervalMs: 10 * 60 * 1000,

    uiStorageKey: 'tmPriorityUiStateV38',
  ...
  function isBridgeReturnGroupItem(item) {
    ...
    return (
      joined.includes('再帰群') ||
      joined.includes('再来群') ||
      joined.includes('検査戻り') ||
      joined.includes('処置戻り') ||
      joined.includes('戻り患者')
    );
  }

  function countFrontWaiting(waitingItems) {
    return waitingItems.filter(item => item.situationText === '受付前で待機').length;
  }

  function countMiddleWaiting(waitingItems) {
    return waitingItems.filter(item => item.situationText === '中待合で待機').length;
  }

  function countReturnGroup(waitingItems) {
    return waitingItems.filter(item => isBridgeReturnGroupItem(item)).length;
  }
  ...
  function renderPanel(waitingItems, examiningItems) {
    ...
    const examiningCount = examiningItems.length;
    ...
    const countSummaryHtml = `
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:4px 10px;font-size:12px;line-height:1.45;margin-bottom:8px;">
        <div style="font-weight:700;color:${CONFIG.colors.examiningStatus};">診察中 ${examiningCount}人</div>
        <div style="font-weight:700;color:${CONFIG.colors.waitingStatus};">診察待 ${waitingCount}人</div>
        <div style="color:#1f2937;">受付前待 ${frontWaitingCount}人</div>
        <div style="color:#1f2937;">中待合待 ${middleWaitingCount}人</div>
        <div style="color:#7c2d12;">再帰群 ${returnGroupCount}人</div>
      </div>
    `;
    ...
    summaryEl.innerHTML =
      `診察待 <b>${topItems.length}</b>名表示 / 全<b>${waitingCount}</b>名` +
    ...
    listEl.innerHTML = countSummaryHtml + topItems.map((item, idx) => {
      const rank = idx + 1;
      const rankColor = getRankColor(rank, item.isUrgentTop);
