...
(function () {
  ...
  function renderWaitingRow(item, rank) {
    ...
    const currentWaitHtml = item.hasReturnMemoText
      ? `⌚今回待${escapeHtml(currentWaitStr)}`
      : '';
    const totalWaitHtml = `<span style="${item.hasReturnMemoText ? 'margin-left:8px;' : ''}color:#7c3aed;">⌚総待ち${escapeHtml(totalWaitStr)}</span>`;
    ...
    html += topItems.map((item, idx) => {
      ...
      return `
        <div title="${escapeHtml(item.detailText)}"
          ...
          <div style="margin-top:4px;font-size:12px;color:#111827;">${escapeHtml(item.departmentText || '')}</div>
          <div style="margin-top:4px;font-size:12px;color:#dc2626;font-weight:700;">${currentWaitHtml}${totalWaitHtml}</div>
